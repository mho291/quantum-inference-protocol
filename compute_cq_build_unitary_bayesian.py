...
def compute_cq_and_unitary_bayesian(
    bits=None, delta=0.001, L=2, bayesian=False, bayesian_runs=1, seed=42
):
    """
    ...
    Arguments:
        bits (np.ndarray):
            1D numpy array of {0, 1} binary time series.

        delta (float):
            Tolerance for state merging. Defaults to 0.001. Delta is automatically
            increased if non-unifilar transitions are detected.

        L (int):
            Length of past for inference.

        bayesian (bool):
            If True, performs Bayesian inference over conditional probabilities using the
            Beta distribution. Defaults to False.

            In this mode, observed transition counts define a Beta posterior over the
            underlying Bernoulli probabilities (via conjugacy with the Binomial likelihood).
            This posterior encodes both the estimated probabilities and their uncertainty
            due to finite data.

        bayesian_runs (int):
            Number of Monte Carlo samples drawn from the Beta posterior. Each sample produces
            one set of conditional probabilities and hence one unitary operator.

            Therefore, this parameter directly controls the number of unitaries generated.
            Each unitary represents one plausible quantum dynamical model consistent with the
            observed data. Ensemble statistics over these unitaries approximate the Bayesian
            predictive distribution. Defaults to 1.

        seed (int):
            Random seed for RNG. Defaults to 42.

    Returns:
        Cq (float):
            Quantum statistical memory.

        Unitary (np.ndarray or list[np.ndarray]):
            If bayesian=False or bayesian_runs=1, returns a single unitary operator.
            If bayesian=True and bayesian_runs>1, returns a list (or array) of unitary
            operators corresponding to posterior samples.

        QMS (np.ndarray or list[np.ndarray]):
            If bayesian=False or bayesian_runs=1, returns an array of inferred quantum memory states.
            If bayesian=True and bayesian_runs>1, returns a list of arrays of inferred quantum memory states.
    ...
    Cqs = []
    QMSs = []
    for idx_bayesian in range(0, bayesian_runs):
        ...
        while True:
            ...
            # ------------------------------------------------------
            # Find / Sampling conditional probabilities
            # ------------------------------------------------------
            ...
            if bayesian:
                A = condCounts[:, 0]
                B = condCounts[:, 1]
                condProbs = np.zeros((np.shape(condCounts)[0], np.shape(condCounts)[1]))

                for ii in range(np.shape(condCounts)[0]):
                    if A[ii] + B[ii] == 0:
                        condProbs[ii, :] = [0.0, 0.0]
                    else:
                        sample = rng.beta(A[ii] + 1, B[ii] + 1, size=1)
                        condProbs[ii, 0] = sample[0]
                        condProbs[ii, 1] = 1 - condProbs[ii, 0]
            else:
            ...
            for ii in range(2**L):
                for jj in range(2**L):
                    ...
                    for kk in range(L):
                        row_to_access = int(
                            mat[kk, 0:L].dot(
                                2 ** np.arange(mat[kk, 0:L].size - 1, -1, -1)
                            )
                        )
                        col_to_access = int(mat[kk, L])
                        prob_from_mat.append(
                            float(condProbs[row_to_access, col_to_access])
                        )
            ...
            for ii in range(np.shape(indices)[0] - 1):
                for jj in range(ii + 1, np.shape(indices)[0]):
                    if (
                        np.dot(probamps[:, indices[ii]], probamps[:, indices[jj]])
                        >= 1 - delta
                    ):
            ...
            windows2 = np.lib.stride_tricks.sliding_window_view(
                bits, window_shape=2 * L
            )
            state_trans = np.zeros(np.shape(windows2)[0])

            for ii in range(np.shape(windows2)[0]):
                idx_r = windows2[ii, :L].dot(
                    2 ** np.arange(windows2[ii, :L].size - 1, -1, -1)
                )
                idx_c = windows2[ii, L:].dot(
                    2 ** np.arange(windows2[ii, L:].size - 1, -1, -1)
                )
        ...
        # Finding / Sampling probability of each unmerged state
        ...
        if bayesian:
            stateProbVec = rng.dirichlet(stateProbCount.flatten() + 1, size=1)
            stateProbVec = stateProbVec.reshape(-1, 1)
        else:
            stateProbVec = stateProbCount / np.sum(stateProbCount)

        # Sqrt-ed conditional futures for the states
        independent_sigma_states = []
        condProbs_concatenated_independent_sigma = np.zeros(
            (2**L, int(np.max(sigma_states)))
        )
        for ii in range(int(np.max(sigma_states))):
            ...
            for jj in range(len(vec)):
                temp = (
                    stateProbVec[vec[jj]] / total_prob_for_that_state
                ) * condProbs_concatenated[:, vec[jj]]
        ...
        windows4 = np.lib.stride_tricks.sliding_window_view(
            bits, window_shape=L
        )  # also == windows3
        stateProbCount = np.zeros((2**L, 1))
        ...
        for ii in range(int(np.max(sigma_states))):
            rho_merged_states += prob_of_independent_sigma[ii] * np.outer(
                probamps_independent_sigma[:, ii], probamps_independent_sigma[:, ii]
            )
        ...
        for ii in range(1, k):
            ...
            for jj in range(0, ii):
                to_minus = (
                    np.dot(UUU[:, ii], UUU[:, jj])
                    / np.dot(UUU[:, jj], UUU[:, jj])
                    * UUU[:, jj]
                )
                for idx, kk in enumerate(
                    to_minus
                ):  # Check for NaN, resulting from division by zero. If NaN, set to zero.
                    if np.isnan(kk):
                        to_minus[idx] = 0
        ...
        for jj in range(int(np.max(sigma_states))):
            condprobs_singletimestep[0, jj] = np.sum(
                condProbs_concatenated_independent_sigma[: 2**L // 2, jj]
            )
            condprobs_singletimestep[1, jj] = np.sum(
                condProbs_concatenated_independent_sigma[2**L // 2 : 2**L, jj]
            )
        ...
        for ii in range(np.shape(temp_transition_matrix)[0]):
            for jj in range(np.shape(temp_transition_matrix)[1]):
                if temp_transition_matrix[ii, jj] == 0:
                    temp_transition_matrix[ii, jj] = (
                        np.nan
                    )  # To prevent np.around(value, 0) = 0 from coinciding with other exact 0 values. This affects finding nextstate_singletimestep.

        temp_condfut_singletimestep = condprobs_singletimestep
        nextstate_singletimestep = np.zeros(
            (
                np.shape(temp_condfut_singletimestep)[0],
                np.shape(temp_condfut_singletimestep)[1],
            )
        )
            ...
            while True:
                ...
                for ii in range(np.shape(temp_condfut_singletimestep)[0]):
                    if temp_condfut_singletimestep[ii, jj] != 0:
                        ...
                        at_row_of_transition_matrix = np.where(
                            np.around(temp_transition_matrix[:, jj], rounding_DP)
                            == np.around(
                                temp_condfut_singletimestep[ii, jj], rounding_DP
                            )
                        )
                        if len(at_row_of_transition_matrix[0]) == 0:
                            ...
                            nextstate_singletimestep[ii, jj] = (
                                at_row_of_transition_matrix[0][0] + 1
                            )
        ...
        for jj in range(num_sigma):  # MATLAB: 1:max(sigma_states)
            ...
            for ii in range(num_sigma):
                for output in range(2):  # MATLAB: 0:1
                    ...
                    Unitary_nonzeros[Unitary_nonzeros_row - 1, Unitary_nonzeros_col] = (
                        Unitary_element
                    )
        ...
        for j in range(startingcol, n):
            Unitary_notyetreorder[:, j] = rng.random(n)
        ...
        Unitaries.append(np.array(Unitary))
        Cqs.append(float(Cq))
        QMSs.append(np.array(QMS))

    if bayesian:

        return Cqs, Unitaries, QMSs
    else:
        Unitary = Unitaries[0]
        Cq = Cqs[0]
        QMS = QMSs[0]
