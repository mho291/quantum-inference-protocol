...
def compute_cq_and_unitary_bayesian(bits=None, delta=0.001, L=2, bayesian=False, bayesian_runs=1):
    """
    ...
    for idx_bayesian in range(0, bayesian_runs):
        ...
        while True:
            ...
            if bayesian:
                ...
                # condProbsVar = np.zeros((np.shape(condCounts)[0], 1))
                
                for ii in range(np.shape(condCounts)[0]):
                    if A[ii] + B[ii] == 0:
                        condProbs[ii, :] = [0.0, 0.0]
                    else:
                        sample = np.random.beta(A[ii]+1, B[ii]+1, size=1)
                ...
                # for ii in range(np.shape(condCounts)[0]):
                #     sample = np.random.beta(A[ii]+1, B[ii]+1, size=1)
                #     # variance = (A[ii] * B[ii]) / ( (A[ii] + B[ii])**2 * (A[ii] + B[ii] + 1) )
                #     # condProbsVar[ii, 0] = variance
                #     condProbs[ii, 0] = sample[0]
                # condProbs[:, 1] = 1-condProbs[:, 0]
            else:
        ...
        if bayesian:
            stateProbVec = np.random.dirichlet(stateProbCount.flatten() + 1, size=1)
