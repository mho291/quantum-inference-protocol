...
def distances_from_Lmin_Lmax(bits, len_min, len_max):
    ...
    for idx, L in enumerate(range(len_min, len_max + 1)):
        ...
        logging.info(f"[{idx+1}/{len_max-len_min+1}] delta = {delta:.12f}, L={L}, trace distance={trace_distance:.6f}")
...
def find_effective_markov_order(bits, len_min, len_max, delta=None):
    ...
    such that all trace distances between p1 and p2, two conditional probabilities
    with the oldest x_{L+1} = 0, 1. 
    ...
    for idx, td in enumerate(trace_distances):
        L = len_min + idx
        if np.all(trace_distances[idx:] <= delta):
